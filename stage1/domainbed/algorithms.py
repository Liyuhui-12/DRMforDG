...
class SCELoss(torch.nn.Module):
    def __init__(self, alpha=0.5, beta=0.6, num_classes=10):
...
# class ERMS1(Algorithm):
#     """
#     Empirical Risk Minimization (ERM)
#     """
#
#     def __init__(self, input_shape, num_classes, num_domains, hparams):
#         super(ERMS1, self).__init__(input_shape, num_classes, num_domains,
#                                   hparams)
#
#         #self.sce=SCELoss(num_classes=num_domains,beta=self.hparams['wbce'])
#         self.featurizer = networks.Featurizer(input_shape, self.hparams)
#         self.classifier = networks.Classifier(
#             self.featurizer.n_outputs,
#             #num_classes,
...
#         self.network=networks.clsdd(input_shape, self.hparams,num_classes,num_domains)
#         print(self.network)
#
#         self.optimizer = torch.optim.Adam(
#             self.network.parameters(),
...
#         #all_c = torch.cat([c for x, y, c in minibatches])
#         all_d = torch.cat([torch.zeros_like(y) + i for i, (x, y) in enumerate(minibatches)])
#
...
#         all_d=all_d[shuffle]
#
#         pre=self.network(all_x,all_y).squeeze()
#         acc = (pre.max(1)[1] == all_d).float().mean()
#         # acc_c = (pre.max(1)[1] == all_c).float().mean()
#         #acc = ((pre > 0.5) * (all_d > 0.5)+(pre < 0.5) * (all_d < 0.5)).float().mean()
#         tmppre=pre.clone()
#         tmppre[:,0],tmppre[:,1]=pre[:,1],pre[:,0]
#
#
#         #prec=all_y.unsqueeze(1)*(tmppre)+(1-all_y.unsqueeze(1))*(pre)
#
#         #acc_c = ((prec > 0.5) * (all_c > 0.5)+(prec < 0.5) * (all_c < 0.5)).float().mean()
#         #acc_c = (prec.max(1)[1] == all_c).float().mean()
#
#         loss = F.cross_entropy(pre, all_d)
#         #loss=self.sce(pre, all_d)
#
#
#
#         #loss = F.l1_loss(pre, all_d)
#
#         self.optimizer.zero_grad()
#         loss.backward()
#         self.optimizer.step()
#
#         #return {'loss': loss.item(),'acc':acc.item(),'acc_c':acc_c.item()}
#         return {'loss': loss.item(), 'acc': acc.item()}
#
#     def predict(self, x,yy):
#         return self.network(x,yy)


class DCRM(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(DCRM, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        ...
        #self.net = nn.Sequential(self.featurizer, self.classifier)
        self.network=networks.dcrms1(input_shape, self.hparams,num_classes,num_domains)
        print(self.network)
