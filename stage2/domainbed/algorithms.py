...
class ERM(Algorithm):
    ...
    def update(self, minibatches, unlabeled=None):
        all_x = torch.cat([x for x, y, xi, yi in minibatches])
        all_y = torch.cat([y for x, y, xi, yi in minibatches])
        all_xi = torch.cat([xi for x, y, xi, yi in minibatches])
        all_yi = torch.cat([yi for x, y, xi, yi in minibatches])
        # all_x=torch.cat((all_x,all_xi))
        # all_y = torch.cat((all_y, all_yi))
        ...
        shuffle = torch.randperm(len(all_x))
        all_y = all_y[shuffle]
        all_x = all_x[shuffle]
        ...
        shuffle = torch.randperm(len(all_xi))
        all_yi = all_yi[shuffle]
        all_xi = all_xi[shuffle]



        pre = self.predict(all_x)
        loss = F.cross_entropy(pre, all_y)

        # acc=(pre.max(1)[1] == all_y).float().mean()



        if len(all_yi)>0:
            prei = self.predict(all_xi)
            lossi = F.cross_entropy(prei, all_yi)
            loss = loss + self.hparams['match_data'] *len(all_yi )/len(all_y)* lossi
        # from IPython import embed
        # embed()
...
class IRM2(Algorithm):
    """Invariant Risk Minimization"""

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(IRM2, self).__init__(input_shape, num_classes, num_domains,
                                   hparams)
...
class VREx2(ERM):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(VREx2, self).__init__(input_shape, num_classes, num_domains,
                                    hparams)
        self.register_buffer('update_count', torch.tensor([0]))
    ...
    def update(self, minibatches, unlabeled=None):
        if self.update_count >= self.hparams["vrex2_penalty_anneal_iters"]:
            penalty_weight = 1.0
        else:
            penalty_weight = 0.0
...
class AbstractMMD0(ERM):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams, gaussian):
        super(AbstractMMD0, self).__init__(input_shape, num_classes, num_domains,
                                           hparams)
        if gaussian:
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        features = [self.featurizer(all_x), self.featurizer(all_xi)]
        classifs = [self.classifier(fi) for fi in features]
        targets = [all_y, all_yi]
...
class AbstractMMD(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        all_x = [x for x, y, xi, yi in minibatches]
        all_y = [y for x, y, xi, yi in minibatches]
        all_xi = [xi for x, y, xi, yi in minibatches]
        all_yi = [yi for x, y, xi, yi in minibatches]
        ...
        for i in range(len(all_x)):
            shuffle = torch.randperm(len(all_x[i]))

            y0 = all_y[i][shuffle]
            x0 = all_x[i][shuffle]

            # shuffle = torch.randperm(len(all_xi[i]))
            # y1 = all_yi[i][shuffle]
            # x1 = all_xi[i][shuffle]
            ...
            xs.append(torch.cat((x0,x1)))
            targets.append(torch.cat((y0,y1)))
            xs0.append(x0)
            targets0.append(y0)
            xs1.append(x1)
            targets1.append(y1)
        ...
        features0 = [self.featurizer(xi) for xi in xs0]
        classifs0 = [self.classifier(fi) for fi in features0]
        features1 = [self.featurizer(xi) for xi in xs1]
        classifs1 = [self.classifier(fi) for fi in features1]
        # features = [self.featurizer(xi) for xi in xs]
        # classifs = [self.classifier(fi) for fi in features]
        # targets = [yi for _, yi in minibatches]
        ...
        for i in range(nmb):
            objective += F.cross_entropy(classifs0[i], targets0[i])
            objective += F.cross_entropy(classifs1[i], targets1[i])*self.hparams['match_data'] *len(targets1[i])/len(targets0[i])
            for j in range(i + 1, nmb):
                fi=torch.cat(features0[i],features1[i])
                fj = torch.cat(features0[j], features1[j])
                penalty += self.mmd(fi, fj)
