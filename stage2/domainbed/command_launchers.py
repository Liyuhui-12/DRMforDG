...
def multi_gpu_launcher(commands):
    ...
    pynvml.nvmlInit()
    gpuhandles=[pynvml.nvmlDeviceGetHandleByIndex(i) for i in range(n_gpus)]

    while len(commands) > 0:
        for gpu_idx in range(n_gpus):
            if gpu_idx not in [0,1,2,3,4,5,6,7]:
                continue
            #info=pynvml.nvmlDeviceGetMemoryInfo(gpuhandles[gpu_idx])
            # if (info.used/info.total)>=0.01:
            #     continue
            ...
            if (proc is None) or (proc.poll() is not None):
                ...
                new_proc = subprocess.Popen(
                    f'CUDA_VISIBLE_DEVICES={gpu_idx+0} {cmd}', shell=True)
